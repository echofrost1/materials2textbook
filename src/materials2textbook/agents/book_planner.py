...
def render_curriculum_order_yaml(book_plan: BookPlan) -> str:
    ...
    order_label = "project_order" if book_plan.metadata.get("textbook_structure") == TEXTBOOK_STRUCTURE else "chapter_order"
    lines = [
...
def render_book_outline_markdown(book_plan: BookPlan) -> str:
    if book_plan.metadata.get("textbook_structure") == TEXTBOOK_STRUCTURE:
        lines = [f"# {book_plan.title} 教材大纲", "", "## 总序", "", "## 前言", ""]
        for chapter in book_plan.chapters:
            lines.extend(
                [
                    f"## 项目{chapter.chapter_no} {chapter.title}",
                    "",
                    "### 项目导学",
                    "",
                    "### 能力图谱",
            ...
            for goal in chapter.learning_goals:
                lines.append(f"- {goal}")
            lines.append("")
            for section_index, section in enumerate(chapter.sections, start=1):
                task_no = section.section_no or f"{chapter.chapter_no}.{section_index}"
                lines.extend([f"### 任务{task_no} {section.title}", ""])
                for module in TASK_MODULES:
                    lines.append(f"#### {module}")
                lines.append("")
                for point_index, point in enumerate(section.knowledge_point_ids, start=1):
                    lines.append(f"- {task_no}.{point_index} {point}")
                lines.append("")
            lines.extend(["### 项目小结", ""])
        return "\n".join(lines).strip() + "\n"
