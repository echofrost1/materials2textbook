...
def _build_rule_ability_graph(
    *,
    ...
    assignments: list[tuple[DigitalBookTask, dict[str, str]]] = [
        (task, _select_ability_domain(task.title, domains)) for task in tasks
    ]
    if not assignments:
        domain = domains[0] if domains else {"id": "domain_01", "label": "\u7efc\u5408\u80fd\u529b"}
        domain_node_id = f"{project_id}_domain_01"
        task_node_id = f"{project_id}_task_01"
        action_node_id = f"{project_id}_action_01"
        assessment_node_id = f"{project_id}_assessment_01"
        nodes.extend(
            [
                {"id": domain_node_id, "column": "ability_domain", "label": domain["label"]},
                {"id": task_node_id, "column": "task", "label": _student_display_title(project_title) or "\u7efc\u5408\u5b66\u4e60\u4efb\u52a1"},
                {"id": action_node_id, "column": "action", "label": "\u68b3\u7406\u5b66\u4e60\u76ee\u6807\u5e76\u5b8c\u6210\u5173\u952e\u77e5\u8bc6\u8bf4\u660e"},
                {"id": assessment_node_id, "column": "assessment", "label": "\u5b66\u4e60\u8bb0\u5f55\u3001\u77e5\u8bc6\u8bf4\u660e\u4e0e\u81ea\u8bc4\u7ed3\u679c"},
            ]
        )
        edges.extend(
            [
                {"from": project_node_id, "to": domain_node_id},
                {"from": domain_node_id, "to": task_node_id},
                {"from": task_node_id, "to": action_node_id},
                {"from": action_node_id, "to": assessment_node_id},
            ]
        )
        return {
    ...
    used_domains: list[dict[str, str]] = []
    for _, domain in assignments:
        if not any(existing["id"] == domain["id"] for existing in used_domains):
            used_domains.append(domain)

    domain_node_ids: dict[str, str] = {}
    for domain_index, domain in enumerate(used_domains, start=1):
    ...
    for task, domain in assignments:
