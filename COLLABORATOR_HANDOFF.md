...
```text
代码：/ai/data/repos/materials2textbook、/ai/data/repos/work-data、/ai/data/repos/work-manuscript
...
```text
/ai/data/repos/materials2textbook/AGENTS.md
```
...
├── .vscode-server/          # VS Code Remote Server，可自动重建
├── .config/                 # 工具配置
├── .codex/                  # Codex 配置和缓存
...
├── tools/                   # 持久化 Python/CLI/缓存工具层
├── config/                  # 持久化 gh 等工具配置
├── models/                  # 模型权重目录
├── model-cache/             # HuggingFace / ModelScope / vLLM 等缓存
├── services/                # 本地服务脚本和运行配置
├── use_ai_env.sh            # 新环境加载持久工具和项目环境变量
└── README_PERSISTENT_ENV.md # 回收后恢复说明
```

目录使用原则：

```text
/root                         只放临时系统状态、编辑器会话和可丢缓存
/ai/data                      放代码、工具、环境、配置、大文件和项目数据
/tmp                          只放临时文件，可随时清理
/ai/data/repos/materials2textbook       管理入口，不做日常开发
/ai/data/repos/work-data                数据处理同伴日常开发
/ai/data/repos/work-manuscript          教材写作同伴日常开发
```
...
```text
/ai/data/repos/materials2textbook   main
/ai/data/repos/work-data            data-pipeline
/ai/data/repos/work-manuscript      manuscript
```
...
`/ai/data/repos/materials2textbook` 保存 worktree 依赖的 Git 元数据，不要直接删除。若以后想取消主 worktree，需要先迁移成 bare Git 布局。
...
1. 不要在 `/ai/data/repos/materials2textbook` 日常开发。
