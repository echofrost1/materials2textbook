...
→ AI 规则对低置信度和跨块素材打标
→ 自动写入 asset_block_map.xlsx，并保留 confidence / relation_type / note 供追溯
...
|---|---|---|---|
| 长视频 | 属于目标大块，可能含多个教学动作 | 转码、ASR、自动/半自动时间码、关键帧 | 不要一开始全自动切片全库视频 |
...
4. 片段表中的 transcript_text、keyframe_paths、evidence_text、boundary_reason 可供系统质量检查和后续追溯。
5. 自动质量规则判定的可用片段比例达到 60% 以上。
...
当前多智能体编排线已经可以直接读取同伴整理的 `asset_block_map.xlsx` 进入整本教材模式，不再要求先做人工审核门槛。

推荐衔接方式：

```text
01_manifest_inventory/asset_block_map.xlsx
→ BookPlannerAgent 生成 book_plan.json / book_outline.md
→ 自动生成 curriculum_order.generated.yml
→ 按章派生 chapter_plan.json
→ 逐章组织证据、正文、视频、案例和练习
→ 汇总 digital_book.json / digital_book.zip
→ open_digital_book.bat 或 scripts/open_digital_book.py 本地预览
```

`asset_block_map.xlsx` 在当前流程中的关键字段：

```text
material_block_cn      作为教材章
knowledge_point_cn     作为节/知识点
asset_id / file_path   用于回到素材和 JSONL 证据
asset_type             用于区分视频、PPT、文档、表格等素材
remark / note          作为低权重补充线索
```

当前系统会按内置焊接课程顺序稳定排序，优先顺序为：
...
未命中上述规则的素材大块不会丢失，会按自动生成的顺序追加到后面，并写入 `curriculum_order.generated.yml`。这个 YAML 是系统生成的课程排序记录；如果后续学校或教材专家给出固定章序，可以把它固化成正式课程模板，让不同机器、不同批次生成保持一致。

需要注意的边界：

- `asset_block_map.xlsx` 负责“素材属于哪章哪节”，不能替代 ASR、PPT 抽取、文档抽取和表格抽取。
- 教材正文仍主要依赖 `02_working_processing/json/*.jsonl` 中的证据文本、视频路径、图片路径和知识点字段。
- 当前项目约定不设置人工审核门槛，系统会自动生成可阅读教材；风险是自动章节顺序、ASR 文本和素材归类不一定完全符合专家最终规划。
- 大素材目录默认只引用原视频和关键帧路径，不复制媒体文件，适合几个 G 到上百 G 的素材工作区。
