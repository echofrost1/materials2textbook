...
def test_export_digital_book_writes_json_viewer_and_assets(tmp_path: Path) -> None:
    ...
    assert any(node["column"] == "task" for node in ability_graph["nodes"])
...
def test_export_digital_book_can_use_llm_generated_ability_graph(tmp_path: Path) -> None:
    response = json.dumps(
        {
            ...
            "columns": [
                ...
                {"id": "ability_domain", "title": "Ability domain"},
                {"id": "task", "title": "Task"},
                {"id": "action", "title": "Action"},
                {"id": "assessment", "title": "Assessment"},
            ],
            "nodes": [
                {"id": "project_01", "column": "project", "label": "Digital equipment project"},
                {"id": "domain_01", "column": "ability_domain", "label": "Equipment inspection"},
                {"id": "task_01", "column": "task", "label": "Know digital equipment"},
                {"id": "action_01", "column": "action", "label": "Explain digital measurement devices"},
                {"id": "assessment_01", "column": "assessment", "label": "Device explanation record"},
            ],
            "edges": [
                {"from": "project_01", "to": "domain_01"},
                {"from": "domain_01", "to": "task_01"},
    ...
    assert any(node["label"] == "Explaindigitalmeasurementdevices" for node in ability_graph["nodes"])
    assert_tree_ability_graph(ability_graph)
    assert ability_graph == book.projects[0].ability_graph
    assert provider.messages
    assert "ability graph" in provider.messages[-1][0]["content"].lower()
